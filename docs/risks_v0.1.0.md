...
| R-CI-04 | Self-reported verification is untrustworthy ("X/X executed" claims) | H | M | High | Eric | Verification must come from a separate actor (different agent, test, or human) or a structural artifact (diff, hash). Never accept the chain's own pass-count as success. | open | 2026-04-24 | docs/reference/april-10-session-analysis.md §3.6 |
