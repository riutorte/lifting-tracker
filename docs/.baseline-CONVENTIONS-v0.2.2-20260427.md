...
- **Operational ledgers (kanban, dispatch-handoff) DO carry semver in the filename**, bumped at sprint boundaries. See §8 for the bump rule. The earlier informal practice of keeping `kanban.md` and `lift-track-dispatch-handoff_v0.1.0.md` as bare ledgers is reversed under v0.2.2 to prevent silent overwrite of historical state.
...
- §6 amended bullet: Operational ledgers (`kanban.md`, `lift-track-dispatch-handoff_v0.1.0.md`) reverse from bare filenames to `<name>_v<version>.md` semver pattern. Reverses the prior "pure ledger" exemption to prevent silent overwrite.
