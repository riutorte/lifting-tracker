...
Per D28 fit-for-purpose: all rows directly named in `lift-track-architecture_v0.4.0.md` Sprint 0a/0b stack are populated. Rows for surfaces that exist in the schema but have no UI in MVP (e.g., `set_videos`, `progress_photo_shares`) are listed with phase tags so the catalog completeness signal is unambiguous.
