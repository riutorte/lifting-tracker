...
|---|---|---|---|---|
| D1 — Primary purpose: log entry + analysis | R | Logging and progress analysis are equally first-class; "entering data without analysis is pointless." | lift-track-architecture_v0.4.0.md §D1 | Renamed from "entry + analysis" — verb-object form: log entry. |
| D2 — Per-set granularity | R | Data model stores individual sets; all stats derived from sets, not pre-computed. | lift-track-architecture_v0.4.0.md §D2 | Subject is Resource (Set). |
| D3 — Hierarchical RBAC, one app | R | Single role-aware app; Athlete → Coach → Gym → Super Admin via inheritance. | lift-track-architecture_v0.4.0.md §D3 | |
| D4 — Cloud DB sole source of truth | R | Supabase Postgres is canonical; devices cache locally for offline. | lift-track-architecture_v0.4.0.md §D4 | |
| D5 — Seeded extensible exercise library | R | Canonical seeded catalog; user-extensible with alias system. | lift-track-architecture_v0.4.0.md §D5 | Renamed from "Seeded + extensible" to remove the conjunctive plus. |
| D6 — Real auth from day one | R | Magic-link email; no anonymous use. | lift-track-architecture_v0.4.0.md §D6 | |
| D7 — Closed 4–6 user alpha | R | Invite-only, named users. No self-serve signup in alpha. | lift-track-architecture_v0.4.0.md §D7 | |
| D8 — Expo with Supabase, offline-first | R | Single codebase iPhone + Web; offline-first via expo-sqlite + Drizzle + TanStack Query + custom Supabase sync adapter (Sprint 0a revision). | lift-track-architecture_v0.4.0.md §D8 | Renamed from "Expo + Supabase" — `+` reads as 'and'. |
| D9 — Free in alpha, subscription later | R | No billing in alpha; `tier` field on users/orgs reserves room for it. | lift-track-architecture_v0.4.0.md §D9 | |
| D10 — Ethan as Coach (inherits Athlete) | R | One account per person; Ethan logs his own training and manages clients. | lift-track-architecture_v0.4.0.md §D10 | |
| D11 — Personal tool to business trajectory | R | Build to product quality; multi-tenant aware; neutral branding. | lift-track-architecture_v0.4.0.md §D11 | Renamed from "Personal tool → business" (arrow OK; kept). |
| D12 — Schema is ontological, not taxonomic | R | Session is atomic; all parents (Program, Routine, Class, Type) are nullable FKs. | lift-track-architecture_v0.4.0.md §D12 | |
| D13 — Training hierarchy above Session | R | Exercise Program → Exercise Type → Routine/Class → Session → Exercise → Set → Reps. | lift-track-architecture_v0.4.0.md §D13 | |
| D14 — Per-implement weight, not total | R | User enters per-implement load; volume math derives total from limb config. | lift-track-architecture_v0.4.0.md §D14 | |
| D15 — Limb Configuration is exercise property | R | `upper_limb_config` and `lower_limb_config` enums on every exercise; variations are distinct entries grouped by family. | lift-track-architecture_v0.4.0.md §D15 | |
| D16 — Rest in integer seconds, three-level cascade | R | Session → exercise → set defaulting; null falls through. | lift-track-architecture_v0.4.0.md §D16 | |
| D17 — Set grouping via group_id | R | Nullable integer on `sets`; same group_id = back-to-back (supersets, drop sets, cluster). | lift-track-architecture_v0.4.0.md §D17 | |
| D18 — Multi-weight notation expands on import | R | WxR → 1 row, WxRxN → N rows, paren group → shared group_id, WxR(big) → one continuous set. | lift-track-architecture_v0.4.0.md §D18 | |
| D19 — Reasoner Duality with Authority Rule | R | Tier 1 deterministic reasoner governs decisions; Tier 2 LLM explains and suggests; Tier 1 wins. | lift-track-architecture_v0.4.0.md §D19 | Renamed from "Reasoner Duality + Authority Rule" — the 'and' implied composition; kept as one decision because the Authority Rule is integral to the duality, not a separate decision. |
| D20 — Watch and wearable apps are separate targets | R | Apple Watch / Wear OS / glasses outside the Expo single-codebase model. Three priority watch features named. | lift-track-architecture_v0.4.0.md §D20 | The "watch and wearable" phrasing is in the source decision text and preserved here verbatim; the row is one decision, not two. Ambiguity flag: read this as "watch+wearable" target-class rather than two compositional concepts. |
| D21 — Goals are first-class entities | R | Seven categories supported in schema (body comp, strength, endurance, skill, habit, health, aesthetic). MVP scope: strength + body weight. | lift-track-architecture_v0.4.0.md §D21 | |
| D22 — Progress photos: private by default, sensitivity-aware | R | Encrypted at rest, EXIF stripped, no gamification, neutral language. | lift-track-architecture_v0.4.0.md §D22 | |
| D23 — Form analysis from video, layered | R | L1 storage + coach review (v2); L2 pose estimation (v3); L3 NL feedback via Reasoner Duality (v3-v4); L4 real-time (v5+). | lift-track-architecture_v0.4.0.md §D23 | |
| D24 — Instructional content from three sources | R | Coach-produced, platform-curated, AI-generated. MVP: text + external link. | lift-track-architecture_v0.4.0.md §D24 | |
...
| Authority Rule | R | Governance principle: Tier 2 LLM cannot override Tier 1 deterministic findings on consequential decisions; user confirmation required for any AI write to athlete data. Per D19. | lift-track-architecture_v0.4.0.md §D19, OV-5c Seq 3, SvcV-1 (Edge Functions) | |
| Reasoner Duality | R | Two-tier AI architecture: Tier 1 deterministic governs decisions; Tier 2 LLM explains and suggests. Per D19. | lift-track-architecture_v0.4.0.md §D19, OV-5c Seq 3 | |
...
| Tier 2 Concern Log | R | Append-only monitoring surface (`.cm/tier2-concerns.json` for governance side; `ai_interactions` enrichment for app side) detecting Authority Rule rubber-stamping signals. Not a gate. Per D19. | lift-track-architecture_v0.4.0.md §D19, OV-5c Seq 3 | |
...
CV-capabilities decomposes the sub-system into 8 themes / 31 epics / 109 features. AV-2 captures themes and the MVP-relevant epics (Type C). Feature-level identifiers (F-numbers) live in `lift-track-themes-epics-features_v0.2.0.md`.
...
| Apply Worktree Isolation | A | Spawn a Cowork-spawned Code task with `isolation: "worktree"` (or invoke interactive Claude Code via `claude --worktree` alias) so agent edits land in `<repo>/.claude/worktrees/<name>/` on a `worktree-<name>` branch and never touch the live tree. Default for any task that may write per orchestration_v0.1.0.md (v0.2.0) §8. | orchestration_v0.1.0.md §8.2 + §8.3, lift-track-destructive-operation-policy_v0.1.0.md §1, retro 0c2 | Sprint 0c2 CC1 deliverable. |
| Approve Per-Worktree | A | Eric reviews a returned worktree path/branch from a completed Code-task child and decides whether to promote (merge) to the live tree. Dispatch does not auto-merge. | orchestration_v0.1.0.md §8.2, lift-track-destructive-operation-policy_v0.1.0.md §3.1 (Tier 2 row) | The asynchronous-review counterpart to Tier-1 synchronous approval. |
| Classify by Destructive-Op Tier | A | Assign an operation a Tier 0–4 risk class based on `(operation, context, target)` per the destructive-operation policy §2; tier determines approval discipline and audit-trail requirements per §3 + §5. | lift-track-destructive-operation-policy_v0.1.0.md §2 + §2.6, retro 0c2 | The discipline that operationalizes the policy. |
| Override Destructive-Op Policy | A | Eric authorizes a bounded bypass of the policy under one of three §4.1 cases (recovery from corruption, urgent production hot-fix, tooling failure); the override lands an incident-doc + operations log + post-mortem + re-evaluation entry per §4.2. | lift-track-destructive-operation-policy_v0.1.0.md §4 (six subsections) | Override without post-mortem is itself a policy failure. |
| Reconstruct Forensic Trail | A | Walk one of the five canonical forensic-query patterns (agent-action-window, file-deletion attribution, table-state-at-time, Tier-1 approval audit, deny-list-bypass check) across git history + session transcripts + kanban + platform audit logs to investigate a Tier 1 / Tier 2 incident. | lift-track-destructive-operation-policy_v0.1.0.md §5.6 + §5.8 | Quarterly drill validates end-to-end reconstructability on a synthetic incident. |
| Bind Strategic Implications | A | Every new ADR / SD / sub-system architecture spec authored from Sprint 0d1 close forward carries a Strategic Implications section (with Security Implications sub-block) naming controls satisfied / partially satisfied / declined / N/A and deviations introduced or closed. | lift-track-kanban-sprint-0d1.md CC-7, lift-track-kanban-sprint-0d2.md CC-4 §4.1 | Architectural-change reflection mechanism. Single ADR template carries both blocks. |
...
Workflows are named multi-step activities with governance posture. Each workflow as a whole is a Capability (C) at the meta-level; its steps are Activities (A). Listed here at the workflow (capability-shaped) level; step decomposition lives in lift-track-source-document-cm_v0.3.0.md §5.1.

| Term | Type | Definition | Source views | Notes |
|---|---|---|---|---|
| WF-001 — Start Session | A | Load context, report state. Concept form: Python exec chain gated by Starter. Target: auto-loaded CLAUDE.md + `cm status` skill invocation. | lift-track-source-document-cm_v0.3.0.md §5.1 | Portfolio scope. Renamed from "Session Start" to verb-object. |
| WF-002 — End Session | A | Save report, persist context, stop. Target: Stop hook + `cm report` skill. | lift-track-source-document-cm_v0.3.0.md §5.1 | Portfolio scope. Renamed from "Session End" to verb-object. |
| WF-003 — Update Document | A | Nine-step doc update with Step-4 GATE. Pattern: chain-with-gate + embedded evaluator-optimizer-verify. Target: `cm update <doc>` skill + pre-commit hook + GitHub Action. | lift-track-source-document-cm_v0.3.0.md §5.1 + §5.5, OV-5c Seq 4 | Portfolio scope. Renamed from "Document Update" to verb-object. |
| WF-003L — Update Document (Light) | A | ~5-step lightweight WF-003 variant for Research-class docs. GATE optional per finding; required for landscape syntheses. | lift-track-source-document-cm_v0.3.0.md §3.7.3 | Portfolio scope. Renamed from "Light Document Update". |
| WF-004 — Reconcile Documents | A | Compare N docs, produce workbook, log gaps. Target: `cm reconcile` skill dispatching to Book Boss `reconcile.py`. | lift-track-source-document-cm_v0.3.0.md §5.1 | Portfolio scope. Renamed from "Reconciliation" to verb-object. |
| WF-005 — Create Document | A | Draft → review → register. Target: `cm new <tier> <id>` skill scaffolds from template + adds manifest entry. | lift-track-source-document-cm_v0.3.0.md §5.1 | Portfolio scope. Renamed from "New Document" to verb-object. |
| WF-006 — Create Skill | A | Design → skeleton → namespace → register. Target: `cm new-skill <id>` over Anthropic skill-creator + manifest entry. | lift-track-source-document-cm_v0.3.0.md §5.1 | Portfolio scope. Renamed from "New Agent / New Skill" — contained 'and / slash list'; collapsed to the dominant target (skill). |
...
|---|---|---|---|---|
| Primer | P | Super Agent | Drop (CLAUDE.md auto-load replaces) | lift-track-source-document-cm_v0.3.0.md §6.0a |
| Starter | P | Meta/Gov (Delegator) | Drop (Claude Code session lifecycle) | lift-track-source-document-cm_v0.3.0.md §6.0a |
| Book Boss | P | Doc Mgmt | Stay as skill script (`book_boss.py`) | lift-track-source-document-cm_v0.3.0.md §6.1, OV-5c Seq 4 |
| Doc Librarian | P | Doc Mgmt | Replace with `.cm/manifest.yaml` + `cm status` | lift-track-source-document-cm_v0.3.0.md §6.0a |
| Notekeeper | P | Meta/Gov (Delegator) | Stay or migrate to GitHub Issues (Q3) | lift-track-source-document-cm_v0.3.0.md §5.6, OV-5c Seq 4 |
| Scorekeeper | P | Meta/Gov | Stay as skill script; append-only `.cm/scorekeeper.json` | lift-track-source-document-cm_v0.3.0.md §6.7, OV-5c Seq 4 |
| Historian | P | Meta/Gov | Replace with `cm history <doc>` over `git log --tags` | lift-track-source-document-cm_v0.3.0.md §5.6, OV-5c Seq 4 |
| Agent Registrar | P | Meta/Gov | Drop (SKILL.md auto-discovery) | lift-track-source-document-cm_v0.3.0.md §5.6 |
| Courier | P | Utility/Export | Drop (git push / releases / tar) | lift-track-source-document-cm_v0.3.0.md §5.6 |
| Timekeeper | P | Meta/Obs | Drop (session report header) | lift-track-source-document-cm_v0.3.0.md §5.6 |
| Reporter | P | Meta/Obs | Stay as skill script; Stop-hook invoked | lift-track-source-document-cm_v0.3.0.md §6.1, OV-5c Seq 4 |
| Agent Architect | P | Meta/Gov | Drop (Anthropic SKILL.md is the open standard) | lift-track-source-document-cm_v0.3.0.md §5.6 |
| Version Coordinator | P | Meta/Gov | Drop (git + semver + tag discipline) | lift-track-source-document-cm_v0.3.0.md §5.6 |
| Instruction Verifier | P | Meta/Gov | Drop (CLAUDE.md is always loaded) | lift-track-source-document-cm_v0.3.0.md §5.6 |
| Context Persistence | P | Meta/Gov | Replace with Claude memory + session reports | lift-track-source-document-cm_v0.3.0.md §5.6 |
| Format Controller | P | Meta/Gov | Replace with pre-commit + PostToolUse hook + CI | lift-track-source-document-cm_v0.3.0.md §5.6, OV-5c Seq 4 |
...
| Bindfs Reaper | P | Post-failure cleanup script that removes stale `.git/*.lock` files and Cowork bindfs-mount remnants left after sandbox-mediated agent failures. Deferred to post-Sprint-0d2 per SD-004; the bypass is the compensating control until the reaper is hardened. | lift-track-destructive-operation-policy_v0.1.0.md §1 (relationship-to-controls table) + §6.3, orchestration_v0.1.0.md §8.6 | Portfolio scope; deferred. Layer in the four-control composition. |
| Reach4All Listener Daemon | P | Python asyncio daemon (`reach4all-listener`) running continuously under macOS LaunchAgent; implements the five-component event-driven architecture (Registry → Fetcher → Normalizer → Deduplicator → Router) over external feeds (Anthropic API changelog, GitHub Releases, NVD CVE, Apple guidelines, vendor terms) and routes events to four-tier action sinks (archive / memory-file / chat / research-task spawn). | lift-track-kanban-sprint-0e.md Workstream A CC-1, CC-2, CC-3 | Portfolio scope; planned Sprint 0e. Closes the DeepSeek-V4-class miss case structurally. |
| Reach4All Scheduler | P | Pull-based, cron-driven research-task framework running ~17 cadence-tiered tasks (daily / weekly / monthly) that complement the listener's push-based event stream. Shares state with the listener through the `~/reach4all/` git-tracked repo. | lift-track-kanban-sprint-0e.md Workstream B | Portfolio scope; planned Sprint 0e. Independently buildable from the listener. |
...
The full standards profile lives in `lift-track-dodaf-StdV-1-standards_v0.1.0.md`. AV-2 indexes the standards Lifting Tracker views actively reference, so a reader can resolve "what is X" without leaving the dictionary. Type R for every row (standards are information artifacts). Sectioned to mirror StdV-1.
...
| Term | Type | Definition | References | Notes |
|---|---|---|---|---|
| Composite tool-selection principle | R | Filter tool/library adoption for MCP-able + self-hostable + not-SaaS + AI-native friction. | lift-track-architecture_v0.4.0.md (Hosting posture); StdV-1 §3 | |
| MCP-first sub-system strategy | R | Every sub-system ships an MCP server before further UI investment. Lifting Tracker ships `lifting-tracker-mcp` after athlete MVP. | lift-track-architecture_v0.4.0.md (cross-cutting principles); SvcV-1 | |
| Three-layer data architecture | R | Transactional (Supabase Postgres) + Vector (pgvector) + Semantic (Apache Jena Fuseki). Mobile reaches Fuseki only via MCP. | lift-track-architecture_v0.4.0.md (cross-cutting principles); OV-1 | |
| Hosting posture | R | Docker containers on Railway. Commitment is to Docker, not Railway. | lift-track-architecture_v0.4.0.md (cross-cutting principles); SvcV-1 | |
| Observability stance | R | HyperDX OSS, ClickHouse-backed, OTel-native. SigNoz fallback. Grafana stack declined on AGPL grounds. | lift-track-architecture_v0.4.0.md (cross-cutting principles); StdV-1 §1 + §3 | |
| Reasoner Duality | R | Tier 1 deterministic governs decisions; Tier 2 LLM explains and suggests. Per D19. | lift-track-architecture_v0.4.0.md §D19; OV-5c Seq 3 | Cross-listed in §2. |
| Authority Rule | R | LLM (Tier 2) cannot override deterministic (Tier 1) findings on consequential decisions. User confirmation required for any AI write to athlete data. Per D19. | lift-track-architecture_v0.4.0.md §D19; OV-5c Seq 3 | Cross-listed in §2. |
| Tier 2 Concern Log | R | Monitoring surface detecting Authority Rule rubber-stamping signals. Not a gate. | lift-track-architecture_v0.4.0.md §D19; StdV-1 §3 | Cross-listed in §2. |
| Four content-drop defenses | R | Fresh-Read Discipline; Post-Mutation Verify; 20K-Token Pre-Flight Cap on Write; Baseline Snapshots. Load-bearing for any session on files >1,500 lines. | lift-track-source-document-cm_v0.3.0.md §5.7 | Portfolio scope. Comma list, not 'and'. |
| Trust-but-verify (16-agent posture) | R | All 16 Concept agents remain operational; Claude Code natives run in parallel as redundant observability; deprecation requires empirical evidence. | lift-track-source-document-cm_v0.3.0.md §6.0a | Portfolio scope. |
...
| Sandboxing Discipline | R | Comprehensive isolation-and-approval discipline combining the deny list (Tier 0 technical refusal), worktree-isolation default (blast-radius containment), destructive-operation policy (authorization + audit framework), and bindfs reaper (post-failure cleanup) to prevent live-tree damage and minimize blast radius from agent actions. The Sprint 0c2 theme. | retro 0c2 (sprint title), orchestration_v0.1.0.md §8 (entire section), lift-track-destructive-operation-policy_v0.1.0.md §1 | Sprint 0c2 deliverable. |
| Layered-Control Composition | R | Four-control governance composition (deny list / worktree default / destructive-op policy / bindfs reaper) where each control is named with what it does and what it does not do; any new control candidate must answer "what does this catch that the existing controls don't" before adopting. | retro 0c2 ("Layered-control framing"), lift-track-destructive-operation-policy_v0.1.0.md §1 (relationship-to-adjacent-controls table) | Canonical pattern for any future Lifting-Tracker-scope destructive-op question. |
| Tier-Based Classification | R | Risk-class governance pattern where operations are named by tier (Tier 0 always-deny → Tier 4 routine), each tier carries an approval discipline and an audit-trail requirement, and the framework is portable across XRSize4 ALL sub-systems by swapping sub-system-specific examples while keeping tier definitions invariant. | lift-track-destructive-operation-policy_v0.1.0.md §2 + §3, retro 0c2 ("Tier-based classification produced a portable framework") | Concept and other sub-systems can reuse the framework. |
| Worktree-Isolation Default | R | The single architectural commitment that "agent edits never touch the live tree." Default sandbox isolation for all code-task spawns from Cowork (`isolation: "worktree"`) and interactive Claude Code CLI sessions (`alias claude='claude --worktree'`); override paths are explicit and auditable (read-only inspection, single-step git ops, doc-only edits). | orchestration_v0.1.0.md §8 (six subsections), lift-track-destructive-operation-policy_v0.1.0.md §1, retro 0c2 | Sprint 0c2 CC1. Pairs with deny list + policy + reaper. |
| Compensating Control | R | Technical or procedural control that mitigates a deviation from a baseline control requirement by accepting residual risk and documenting the tension between the baseline requirement and the accepted mitigation; carries explicit residual-risk + re-evaluation-trigger blocks and cross-references the deviation it covers. DEV-001 (bindfs reaper) and DEV-002 (`GIT_OPTIONAL_LOCKS=0`) are the load-bearing examples. | lift-track-destructive-operation-policy_v0.1.0.md §6.2 (SD-004 reference), lift-track-kanban-sprint-0d1.md CC-3 | Portfolio scope. Cited locally because the policy is one of the controls in the SD-009 baseline. |
| Solo+AI Approval Matrix | R | Two-actor approval discipline (Eric + Claude) at current scale where Eric is the sole approver for Tier 1 (synchronous) and Tier 2 (asynchronous diff review); Tier 3 is implicit-review-via-audit-trail; Tier 4 has no approval. Multi-approver path is named for future Fernando onboarding rather than over-engineered today. | lift-track-destructive-operation-policy_v0.1.0.md §3.1 + §3.4, retro 0c2 ("Solo+AI scale with named expansion path") | Right-sized governance per global CLAUDE.md "enterprise rigor is selective." |
| Decision-History Traceability | R | Discipline that ensures every architectural decision, strategic decision, and material operational change is explicitly cross-referenced in source control (ADRs, strategic-decisions-log, D-decision amendments) so a future reader can walk the decision chain and understand the reasoning. | lift-track-kanban-sprint-0d2.md CC-4 §4.6, retro 0c1 (improvement that propagated to 0d2) | Binds from Sprint 0d2 close per CONVENTIONS §11.9 amendment. |
| Audit-Class Artifact (version-suffix rule) | R | Operational or Reference-class document that carries a version-suffix in its filename (semver or date-based) because it is load-bearing for reproducibility and audit trails (CONVENTIONS, ADRs, destructive-op policy, security controls baseline, reference architecture). Contrasted with transient operational docs (kanbans, retros, dispatch handoffs) frozen at sprint/session close that do not carry version-suffix in filename. | lift-track-kanban-sprint-0d2.md CC-4 §4.5, CONVENTIONS_v0.2.4.md §11.8 (binding amendment) | Binds from Sprint 0d2 close. |
| Three-Tier DoDAF View Layering | R | DoDAF view-layering discipline where Reference → Platform → Sub-system is the three-tier abstraction, each tier produces the same DoDAF view set, cross-references layer downward, and compliance flows upward. Distinct from "Three-layer data architecture" (which is a Postgres / pgvector / Fuseki data-tier concept). | lift-track-kanban-sprint-0d2.md CC-2.3 + CC-4 §4.4, CONVENTIONS_v0.2.4.md §11.7 (binding amendment) | Eric's 2026-04-29 correction. Binds from Sprint 0d2 close. |
| Strategic Implications Block | R | Mandatory section in every ADR / SD / sub-system architecture spec authored from Sprint 0d1 close forward; carries a Security Implications sub-block naming controls satisfied / partially satisfied / declined / N/A per applicable framework families plus deviations introduced or closed plus sub-system applicability. Single ADR template carries both Strategic and Security Implications. | lift-track-kanban-sprint-0d1.md CC-7, lift-track-kanban-sprint-0d2.md CC-4 §4.1 | Portfolio scope. Binds from Sprint 0d1 close per OQ3 + Sprint 0d2 CC-4 §4.1 ADR template. |
| Micro-Sprint Cadence | R | Intra-day open-and-close sprint pattern (1 calendar day, 1–2 CCs) valid per CONVENTIONS_v0.2.4 §14.3. Criterion: scope is pre-staged in the prior sprint's kanban, deliverables are tightly-coupled, and one calendar day is realistic with no unknowns. Sprint 0c0.5 and Sprint 0c2 are the two clean instances. | retro 0c2 (sprint stats + key takeaway 3), CONVENTIONS_v0.2.4.md §14.3 | |
...
| Term | Type | Definition | References | Notes |
|---|---|---|---|---|
| Content Class — Architecture | R | ADRs, lift-track-architecture_v0.4.0.md, concept docs, ontology plans. WF-003 full. Semver. GATE required. No staleness. | CONVENTIONS_v0.2.4 §2 | |
...
| Content Class — Code (deferred) | R | Source, tests, build scripts, IaC. Governance deferred to future code-cm discipline. | lift-track-source-document-cm_v0.3.0.md §3.7 | |
...
| Term | Type | Definition | References | Notes |
|---|---|---|---|---|
| Tier — REFERENCE [document] | R | Foundational; describes *what things are* and the vocabulary other docs use. | lift-track-source-document-cm_v0.3.0.md §3.2 | Bracket-disambiguated from Reasoner Tier 1/2. |
| Tier — COMPANION [document] | R | Deep-dive; describes *how things work* in a specific dimension. | lift-track-source-document-cm_v0.3.0.md §3.2 | |
| Tier — MASTER [document] | R | Integration; the single doc a newcomer reads. Authority lives in the tier it integrates from. | lift-track-source-document-cm_v0.3.0.md §3.2 | |
| Tier — OPERATIONAL [document] | R | Live; describes the *current state* of the system. Shortest half-life. | lift-track-source-document-cm_v0.3.0.md §3.2 | |
...
## Section 16 — Risk identifiers (sample; full register at `lift-track-risks_v0.1.0.md`)
...
|---|---|---|---|
| R-CI-01 | R | Content drops via micro_compact during document writes. High severity. Open. | lift-track-risks_v0.1.0.md §3.1, OV-5c Seq 4 |
| R-CI-02 | R | 25K-token Read ceiling throws on large docs. Medium severity. Open. | lift-track-risks_v0.1.0.md §3.1 |
| R-CI-04 | R | Self-reported verification is untrustworthy. High severity. Open. | lift-track-risks_v0.1.0.md §3.1 |
| R-TV-01 | R | GitHub pay-to-play migration window closes. High severity. | lift-track-risks_v0.1.0.md §3.2, StdV-1 §2 |
| R-TV-03 | R | HyperDX relicense risk. Tracked; SigNoz is named fallback. | lift-track-risks_v0.1.0.md, StdV-1 §2 |
| R-TV-04 | R | `@hyperdx/otel-react-native` SDK abandonment risk; raw OTel fallback path documented. | lift-track-risks_v0.1.0.md, SV-1 |
| R-TV-05 | R | Railway commercial-terms shift; Hetzner / Fly / self-hosted Kubernetes are migration targets. | lift-track-risks_v0.1.0.md, SvcV-1 |
| R-TV-06 | R | ClickHouse relicense risk affecting HyperDX backend. | lift-track-risks_v0.1.0.md, StdV-1 §2 |
| R-PT-04 | R | MCP version churn risk. Tracked. | lift-track-risks_v0.1.0.md, StdV-1 §1 |
| R-GV-04 | R | CC-017 — governance step omission under content pressure. High severity. | lift-track-risks_v0.1.0.md §3.4 |
| R-SD-01 | R | Tier 2 Concern Log not yet instrumented. High severity. | lift-track-risks_v0.1.0.md §3.7, StdV-1 §3 |

(Full register: `docs/lift-track-risks_v0.1.0.md`. Additional rows promoted into AV-2 as risks become cross-view-referenced.)

---

## Section 17 — User-story prefixes (catalog at `lift-track-user-stories_v0.2.0.md`)

Type R. Only ID-range conventions are catalogued; individual story IDs are looked up in the canonical doc.

| Term | Type | Definition | References |
|---|---|---|---|
| US-0XX | R | Athlete-facing MVP stories. | lift-track-user-stories_v0.2.0.md Phase 1 |
| US-1XX | R | Coach + training-structure + goals-expansion + form-capture stories (v2). | lift-track-user-stories_v0.2.0.md Phase 2 |
| US-2XX | R | Admin / Gym / Teams stories (Future). | lift-track-user-stories_v0.2.0.md (Future phases) |
| US-3XX | R | NFR-class stories: performance, security, privacy, sync, data portability. | lift-track-user-stories_v0.2.0.md (Cross-cutting NFRs) |
...
|---|---|---|---|
| AV-1 — Overview | R | Charter-level orientation: portfolio + sub-system + alpha scope. | lift-track-dodaf-AV-1-overview_v0.1.0.md |
| AV-2 — Integrated Dictionary | R | Term registry (this file). | lift-track-dodaf-AV-2-dictionary_v0.3.0.md |
| CV — Capability Viewpoint | R | Capability hierarchy: themes, epics, MVP slice, anchors. | lift-track-dodaf-CV-capabilities_v0.1.0.md |
| OV-1 — Concept Graphic | R | Single high-level operational picture: portfolio, sub-systems, infrastructure, externals, clients. | lift-track-dodaf-OV-1-concept-graphic_v0.1.0.md |
| OV-5c — Activity Sequence | R | Time-ordered behavior across four load-bearing sequences (log workout offline-online; assign routine; Reasoner Duality; WF-003 GATE). | lift-track-dodaf-OV-5c-activity-sequence_v0.1.0.md |
| SV-1 — Interfaces | R | Component diagram, component catalog, port catalog. | lift-track-dodaf-SV-1-interfaces_v0.1.0.md |
| SV-6 — Data Exchanges | R | Row-by-row data exchange catalog across component boundaries. | lift-track-dodaf-SV-6-data-exchanges_v0.1.0.md |
| SvcV-1 — Services Context | R | Service-shaped surface: providers, consumers, contracts, composite-principle status. | lift-track-dodaf-SvcV-1-services-context_v0.1.0.md |
| DIV-2 — Logical Data | R | Entity-relationship picture of the data model. | lift-track-dodaf-DIV-2-logical-data_v0.1.0.md |
| StdV-1 — Standards | R | Standards profile: external, vendor, internal, declined, watched. | lift-track-dodaf-StdV-1-standards_v0.1.0.md |

---

## Section 19 — Operational governance controls (Sprint 0c2+)

The four-control composition that governs destructive-operation risk on Lifting Tracker. Each control answers "what does this catch that the other controls don't." Type R for governance information artifacts; bindfs reaper has a corresponding component row in §9a (the artifact here is the policy / specification, not the script). Portfolio-scope controls (Security Controls Baseline, AAF, Reference Architecture) are flagged.

### 19a — Four-control composition

| Term | Type | Definition | References | Notes |
|---|---|---|---|---|
| Deny List (CC9) | R | Tier-0 implementation: ~70 literal bash patterns refused at Claude Code's permission gate before any human review (path-resolution destruction, git-history destruction, cloud-platform destroy, database wipe, secret exfiltration, permission destruction, pipe-to-shell). Lives in `~/lifting-tracker/.claude/settings.json`. | Sprint 0c1 CC9, lift-track-destructive-operation-policy_v0.1.0.md §2.1, orchestration_v0.1.0.md §8.6 | The deepest layer; refusal logs at the deny-list layer for upstream prompt review. |
| Worktree-Isolation Default | R | Sprint 0c2 CC1 deliverable: `isolation: "worktree"` for Cowork-spawned Code tasks plus `alias claude='claude --worktree'` for interactive Mac terminal sessions; named override patterns for read-only inspection, single-step git ops, and doc-only edits. | orchestration_v0.1.0.md §8 (six subsections), lift-track-destructive-operation-policy_v0.1.0.md §1 | Cross-listed in §13. |
| Destructive-Operation Policy | R | Sprint 0c2 CC2 deliverable: tier-based authorization and audit framework over Lifting Tracker's destructive-operation surface (Cowork-spawned Code tasks, interactive Claude Code sessions, direct-to-platform actions on Supabase / Vercel / GitHub / EAS). | lift-track-destructive-operation-policy_v0.1.0.md (entire doc), retro 0c2 | Reference-class artifact; governance-as-code layer. |
| Bindfs Reaper (deferred) | R | Post-failure cleanup script removing stale `.git/*.lock` files and Cowork bindfs-mount remnants. Deferred post-Sprint-0d2 per SD-004; until then, the bypass (`GIT_OPTIONAL_LOCKS=0`, DEV-002) is the compensating control. | lift-track-destructive-operation-policy_v0.1.0.md §1 + §6.3, SD-004 (portfolio scope) | Component row in §9a; specification row here. |

### 19b — Destructive-operation tier framework

Five tiers ordered from "always blocked at the technical layer" to "no restriction needed." Each tier carries an approval discipline and an audit-trail requirement. The framework is project-specific in its examples but portable across XRSize4 ALL sub-systems.

| Term | Type | Definition | References | Notes |
|---|---|---|---|---|
| Tier 0 — Always-deny | R | Operations refused at the deny-list layer before any human review (`rm -rf /`, `git push --force`, `terraform destroy`, `supabase db reset`, `cat .env*`, `chmod -R 777 ~`, `curl * \| bash`, etc.). No approval can override; pattern-match frequency informs upstream prompt review. | lift-track-destructive-operation-policy_v0.1.0.md §2.1 | |
| Tier 1 — High-risk, manual approval | R | Technically reversible but operationally costly; Eric's explicit synchronous approval required; per-operation, not per-class. Examples: production Supabase schema migration, force-rotating production credentials, restoring from PITR, deleting GitHub branch with unmerged work, mass-delete >100 files. | lift-track-destructive-operation-policy_v0.1.0.md §2.2 + §3.1, retro 0c2 | Approval log per §2.2 format; kanban + session transcript. |
| Tier 2 — Medium-risk, peer-review | R | Individually reversible but cumulatively risky; Eric reviews diff before merge; lands in worktree first. Examples: refactor 5–15 files, dependency upgrade 1–5 files, RLS policy addition, deny-list entry addition, edge-function deployment. | lift-track-destructive-operation-policy_v0.1.0.md §2.3 + §3.1 | Diff is the unit of review; PR or worktree-with-diff is the format. |
| Tier 3 — Low-risk, audit-trail required | R | Routine operations where the audit trail itself is the control: conventional commit message, git history, kanban entry. Examples: single-file edit, local-branch git ops, test runs, lint/format passes, OPERATIONAL/REFERENCE doc edits. | lift-track-destructive-operation-policy_v0.1.0.md §2.4 + §3.1 | Audit is automatic via existing tooling. |
| Tier 4 — No restriction | R | Read-only operations with no destructive risk: Read/Grep/Glob, status checks, dry-run lint, plan generation, WebFetch, listing sessions. No approval, no extra audit. | lift-track-destructive-operation-policy_v0.1.0.md §2.5 | A chain of Tier 4 reads producing a Tier 1 conclusion still triggers Tier 1 discipline at the consequential action. |
| Cross-Tier Movement | R | Tiers are a function of `(operation, context, target)` and operations move tiers as context changes (target moves local→shared, staging→production, single→batch; or downward: dry-run, disposable worktree, recovery action). Default to higher tier when ambiguous. | lift-track-destructive-operation-policy_v0.1.0.md §2.6 | Practical rule. |

### 19c — Approval matrix (solo+AI scale)

| Term | Type | Definition | References | Notes |
|---|---|---|---|---|
| Approval Matrix — Solo+AI | R | Two-actor matrix at current scale: Tier 1 = Eric synchronous-approval; Tier 2 = Eric asynchronous diff-review; Tier 3 = implicit review via audit trail; Tier 4 = none. Conditions modify (production-class data raises tier; disposable worktree lowers; autonomous-mode prevents Tier 1 execution). | lift-track-destructive-operation-policy_v0.1.0.md §3.1 + §3.2 + §3.3 worked-examples | Eight worked examples in §3.3. |
| Approval Matrix — Multi-Approver (placeholder) | R | Placeholder shape for post-Fernando-onboarding: domain-split approver assignment (Eric owns architecture / data model; Fernando owns content / coach UX); cross-domain Tier 1 ops require both. Crystallizes when Fernando role definition lands. | lift-track-destructive-operation-policy_v0.1.0.md §3.4 | SD-012 Lifecycle Integrity re-evaluation event. |

### 19d — Override procedure and audit-trail

| Term | Type | Definition | References | Notes |
|---|---|---|---|---|
| Override Procedure | R | Bounded-bypass mechanism for three named situations only: recovery from corruption, urgent production hot-fix, tooling failure. Each override fires an incident-doc + operations log + post-mortem + re-evaluation entry; "speed" / "asleep" / "precedent" / "agent confidence" are not valid overrides. | lift-track-destructive-operation-policy_v0.1.0.md §4 (six subsections) | Override without post-mortem is a policy failure. |
| Audit-Trail Requirement | R | Per-tier reconstructable trail: Tier 0 captured at deny-list layer; Tier 1 has approval record + kanban entry + operations log + PITR/backup window + outcome verification; Tier 2 has diff + test pass + kanban + reviewer identity; Tier 3 has conventional commit + commit author + kanban; Tier 4 needs no extra audit. | lift-track-destructive-operation-policy_v0.1.0.md §5 (eight subsections) | Reconstruction depends on shortest retention window in the chain. |
| Forensic-Query Pattern | R | Five canonical reconstruction patterns: (1) agent-action-window between time A and B; (2) file-deletion attribution; (3) table-state at time T (Supabase PITR); (4) Tier-1-approval-logged check; (5) deny-list-bypass check. Each pattern walks git + session transcripts + kanban + platform audit logs. | lift-track-destructive-operation-policy_v0.1.0.md §5.8 | Quarterly drill validates end-to-end reconstructability. |
| Re-Evaluation Trigger | R | Five-class trigger taxonomy for the policy (and structurally similar artifacts): Type 1 incident-driven (post-mortem closes by updating the policy); Type 2 scope expansion (production launch, Fernando onboarding); Type 3 Anthropic platform change; Type 4 adjacent control lands; Type 5 quarterly review (floor). Each trigger names the specific events. | lift-track-destructive-operation-policy_v0.1.0.md §7 (four subsections) | Per SD-012 Lifecycle Integrity. |

### 19e — Portfolio-scope governance referenced from Lifting Tracker

These rows are portfolio-scope artifacts cited here because Lifting Tracker views and policies actively reference them. Migrate to platform-level AV-2 when xrsize4all repo stands up (Sprint 0d2).

| Term | Type | Definition | References | Notes |
|---|---|---|---|---|
| Security Controls Baseline (SD-009) | R | Portfolio-level v0.1.0 reference cataloging 16 frameworks (NIST 800-53 r5, CSF 2.0, 800-128, SSDF, ISO 27001:2022 / 27034 / 25010, MITRE ATT&CK / CWE / CAPEC / D3FEND, OWASP ASVS / DSOMM / LLM Top 10, CIS v8, DISA STIG) with explicit ADOPTED / PARTIAL / DECLINED / N/A posture per applicable control plus the compensating-control register (DEV-001 / DEV-002). | lift-track-kanban-sprint-0d1.md (entire sprint), lift-track-destructive-operation-policy_v0.1.0.md §6.2 + §6.5 | Portfolio scope (SD-009). The destructive-op policy is one row-level entry under it. |
| Production-Readiness Gate Matrix | R | Lifting Tracker's 31-row control set tracking each control's required-status-at-gate / current-posture / closure-plan / sprint-placement. Operationalized as `lift-track-production-readiness-controls_v0.1.0.md`; acts as the operator's gate-evaluation checklist when Lifting Tracker approaches D7 alpha → public. | lift-track-kanban-sprint-0d1.md CC-4, security-controls-baseline §3.16a.A | Sprint 0d1 deliverable. |
| Agentic Architecture Framework (AAF) | R | Portfolio-level architectural framework renamed from AIAF in Sprint 0d2 per SD-006; establishes seven design patterns (P1 Identity / P2 Ontology+Data / P3 Intelligence / P4 Content / P5 Commerce / P6 Analytics / P7 Lifecycle) and four architectural invariants. DoDAF view set is one component within AAF, not the whole framework. | lift-track-kanban-sprint-0d2.md CC-1 + CC-2.2 (SD-006), CONVENTIONS_v0.2.4.md §11 (reframe pending) | Portfolio scope; canonical entry migrates to platform-level AV-2 at xrsize4all standup. |
| Reference Architecture (SD-007) | R | Portfolio-level v0.1.0 architecture template establishing the seven-pattern catalog, per-sub-system instantiation template, cross-cutting capability catalog (eight capabilities), four architectural invariants, specialization guidelines, and inheritance model. Lands at `xrsize4all://docs/architecture/xrsize4all-reference-architecture_v0.1.0.md`. | lift-track-kanban-sprint-0d2.md CC-2 (SD-007) | Portfolio scope; substrate for sub-system #2 onboarding. |
...
**User stories referenced:** prefix-level only (US-0XX, US-1XX, US-2XX, US-3XX); individual story IDs catalogued in `lift-track-user-stories_v0.2.0.md`.

**Other DoDAF views referenced (this directory):** AV-1, CV-capabilities, OV-1, OV-5c, SV-1, SV-6, SvcV-1, DIV-2, StdV-1.

**Cross-cutting governance references:** CONVENTIONS_v0.2.4.md §2 (content classes), §6 (markdown standard), §7 (frontmatter), §8 (semver), §10 (baseline-snapshot discipline), §11 (DoDAF view set, fit-for-purpose, SysML iconography). ADR D28 (DoDAF + SysML iconography). lift-track-source-document-cm_v0.3.0.md §1.4 + §3.2 + §3.7 + §4.9 + §5.1 + §5.5 + §5.7 + §6.0a + §6.1 + §6.6 + §6.7 + §6.8 (portfolio governance terms cited in this dictionary).

**Sprint of last revision:** Sprint 0c2 close (2026-04-30). Additive enrichment from Sprint 0c2 work and adjacent prep deliverables (0d / 0d1 / 0d2 / 0e kanbans). MINOR bump per CONVENTIONS §8 (additive content; no v0.2.0 row removed).
...
| 0.2.0 | 2026-04-29 | Sprint 0c1 CC8 deliverable. Inclusion-completeness pass against all 9 sibling DoDAF views (AV-1, CV-capabilities, OV-1, OV-5c, SV-1, SV-6, SvcV-1, DIV-2, StdV-1). CARP/CARPO type column added per row. Four authoring rules applied (verb-object activities, no-and, synonym-collapse, bracketed-context-tag). Domain concepts added as standalone rows: Session, Exercise, Set, Limb Configuration, Volume, Rest Cascade, Goal, Progress Photo, Group ID, Implement, Reasoner Tier 1, Reasoner Tier 2 (and surfaced concepts: Workout Episode, AI Interaction, Magic-Link Auth, JWT, RLS, PostgREST, Outbox, Last-Write-Wins, Optimistic Update, WAL, ASR, Dead-Letter Queue, Body Weight, Routine, Class [exercise], Program, Coach-Athlete Assignment, Routine Assignment, Authority Rule, Reasoner Duality, AI-Generated Content, RPE, Side, Rep, Goal Milestone, Goal Progress Snapshot, Set Video, Set Video Annotation, Set Video Analysis, Exercise Family, Exercise Type, Exercise Alias, Exercise Content, Biometric Sample, Program Component, Weight Entered, Implement). Reframed scope as LIFTING TRACKER sub-system (vs portfolio-level XRSize4 ALL); platform-scope rows flagged. Tier corrected from OPERATIONAL to ARCHITECTURE; status promoted from draft to accepted. New sections: §2 Domain concepts, §3 Capabilities, §4 Activities, §9 Components and ports, §10 Services, §11 Data entities, §12 Standards index, §18 DoDAF views. Structural revision per CONVENTIONS §8 Architecture-class semver. Baseline of v0.1.0 captured at `.baseline-lift-track-dodaf-AV-2-v0.1.0-20260429.md`. |
| 0.3.0 | 2026-04-30 | Additive enrichment from Sprint 0c2 work and adjacent prep deliverables (0d / 0d1 / 0d2 / 0e kanbans). MINOR bump per CONVENTIONS §8 (additive content; no v0.2.0 row removed; existing entries preserved verbatim). New domain concepts (28 total): §4 Activities adds Apply Worktree Isolation, Approve Per-Worktree, Classify by Destructive-Op Tier, Override Destructive-Op Policy, Reconstruct Forensic Trail, Bind Strategic Implications. §9a Components adds Bindfs Reaper (deferred), Reach4All Listener Daemon (planned Sprint 0e), Reach4All Scheduler (planned Sprint 0e). §13 Architectural principles adds Sandboxing Discipline, Layered-Control Composition, Tier-Based Classification, Worktree-Isolation Default, Compensating Control, Solo+AI Approval Matrix, Decision-History Traceability, Audit-Class Artifact (version-suffix rule), Three-Tier DoDAF View Layering, Strategic Implications Block, Micro-Sprint Cadence. NEW §19 — Operational governance controls — captures the Sprint-0c2 four-control composition (deny list / worktree default / destructive-op policy / bindfs reaper), the destructive-operation tier framework (Tier 0–4 plus Cross-Tier Movement), the solo+AI approval matrix (with multi-approver placeholder), the override procedure with audit-trail / forensic-query / re-evaluation-trigger discipline, and four portfolio-scope governance references (Security Controls Baseline / Production-Readiness Gate Matrix / Agentic Architecture Framework / Reference Architecture). Source documents consumed: lift-track-destructive-operation-policy_v0.1.0.md (Sprint 0c2 CC2); orchestration_v0.1.0.md §8 (in-place v0.2.0 amendment, Sprint 0c2 CC1); retro-0c2; kanban-sprint-0d / 0d1 / 0d2 / 0e (prep deliverables for upcoming sprints). Structural revision per CONVENTIONS §8 Architecture-class semver. |
