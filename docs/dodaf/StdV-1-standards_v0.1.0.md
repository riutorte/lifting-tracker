...
| **YAML 1.2** | Frontmatter and manifest format | All in-scope docs (frontmatter); `.cm/manifest.yaml` | Required | Per CONVENTIONS_v0.2.2.md §7. |
| **Markdown (CommonMark + GitHub-flavored extensions)** | Document format | All in-scope docs | Required | Per CONVENTIONS_v0.2.2.md §6 (extension `.md`). |
...
| **SemVer 2.0.0** | Version numbering for documents and software | Architecture-class docs; Reference-class docs; software releases | Required | Per CONVENTIONS_v0.2.2.md §8. |
...
| **SLSA v1.0** (Supply-chain Levels for Software Artifacts) | Build provenance attestation framework | Skill-script supply chains; immutable git tags as content-addressed identifiers | Adopted (Level 1 implicit; L2 deferred) | Per source-doc-cm-design.md §2.5 (immutable artifacts) + §6.7 (tamper-evidence scorekeeper as solo-scale equivalent). G12/G13 deferred. |
| **CycloneDX v1.6** (SBOM format) | Software Bill of Materials | Skill-package SBOMs generated on every baseline tag | Adopted | Per source-doc-cm-design.md §5.2 step 8. `cyclonedx-py` is the generator. |
| **sigstore / cosign / gitsign** | Cryptographic artifact and commit signing | Future portfolio expansion | Watched | Per source-doc-cm-design.md §6.7. Adoption deferred until second contributor joins; sigstore is layered onto the existing scorekeeper schema without breaking changes. |
| **gitleaks** | Secret scanning | Pre-commit + CI | Adopted | Staged six-step rollout per source-doc-cm-design.md §5.2. |
...
| **W3C PROV-DM / DataOps** | Data provenance taxonomy | `data/combined_workout_log.txt` import + future external data ingestion | Adopted (out-of-scope for document-cm; in-scope for data lineage) | Per source-doc-cm-design.md scope note (G28). |
...
| **Apple HealthKit framework spec** | Biometrics ingestion on iOS | Platform Biometrics sub-system (deferred) | Required when HealthKit ingestion lands | Per architecture_v0.4.0.md non-decisions. |
...
| **NativeWind** (open source; MIT) | Tailwind utility classes on React Native | MobileClient styling | Required | Per source-doc-cm-design.md §6.8. |
...
| **Anthropic Agent Skills format (SKILL.md)** | Skill packaging for Claude Code, Cursor, Codex CLI, Antigravity, Gemini CLI | document-cm and future skills | Adopted | Open standard published by Anthropic Dec 2025; cross-host portability per source-doc-cm-design.md §1.4. |
...
|---|---|---|---|---|
| **D19 Reasoner Duality + Authority Rule** | Two-tier AI architecture: Tier 1 deterministic governs decisions; Tier 2 LLM explains and suggests | Every AI feature in the portfolio | Required | architecture_v0.4.0.md §D19 |
| **Composite tool-selection principle** | Filter for MCP-able + self-hostable + not-SaaS + AI-native friction | Every tool/library adoption decision | Required | architecture_v0.4.0.md (D8 revision; Hosting posture) |
| **MCP-first sub-system strategy** | Each XRSize4 ALL sub-system ships an MCP server before further UI investment | Lifting Tracker (`lifting-tracker-mcp`); document-cm; future sub-systems | Required | architecture_v0.4.0.md (Cross-cutting principles); source-doc-cm-design.md §6.6 |
| **Three-layer data architecture** | Transactional (Postgres) + Vector (pgvector) + Semantic (Fuseki via MCP) | Portfolio data layer | Required | architecture_v0.4.0.md (Cross-cutting principles) |
| **Hosting posture** | Docker containers on Railway; commitment to Docker, not Railway | All self-hosted services | Required | architecture_v0.4.0.md (Cross-cutting principles) |
| **Observability stance** | HyperDX OSS, ClickHouse-backed, OTel-native; SigNoz fallback; Grafana stack declined on AGPL grounds | All telemetry exchanges | Required | architecture_v0.4.0.md (Cross-cutting principles); R-TV-03 |
| **Four content-drop defenses** | Fresh-Read Discipline; Post-Mutation Verify; 20K-Token Pre-Flight Cap on Write; Baseline Snapshots | Any session operating on docs >1,500 lines | Required | source-doc-cm-design.md §5.7 |
| **Trust-but-verify (16-agent posture)** | All 16 Concept agents remain operational; Claude Code natives run in parallel as redundant observability; deprecation requires empirical evidence via Tier 2 concern log | Concept agent suite vs Claude Code primitives | Required | source-doc-cm-design.md §6.0a |
| **Brain-hands separation** | Reasoning (LLM) and execution (skill scripts, hooks, CI) are decoupled; interfaces outlast implementations | Every skill design | Required | source-doc-cm-design.md §1.4 |
| **Generator-evaluator separation** | Writer cannot be the verifier; Book Boss verify is structurally separate from the writer | Every WF-003 invocation | Required | source-doc-cm-design.md §1.4, §5.5 |
| **Composition discipline (CC-017)** | Governance steps must be composed (skill calls a script that runs the check), not listed in prose | Every load-bearing rule in the CM discipline | Required | source-doc-cm-design.md §1.4, §5.4 |
| **Tier 2 concern log** | Monitoring surface (`.cm/tier2-concerns.json`) detecting Authority Rule rubber-stamping signals | All Tier 2 LLM outputs | Required | architecture_v0.4.0.md §D19; R-GV-01; R-SD-01 |
| **GATE artifact triple (request / assignment / resolution + timeout + reassignment)** | Three-record audit structure for every WF-003 GATE invocation | document-cm GATE flows | Required | source-doc-cm-design.md §5.5a |
| **Tamper-evidence scorekeeper (prev_hash chain)** | SHA-256 chained append-only ledger | `.cm/scorekeeper.json` | Required | source-doc-cm-design.md §6.7 |
| **Hybrid `ai_interactions` schema** | Always-populated `input_raw` + typed `output_episode` | AI_INTERACTIONS table writes | Required | source-doc-cm-design.md §4.9 + §6.8 |
| **Closed-vocabulary feedback labels** | Fixed positive/negative label sets for AI/governance feedback | document-cm feedback surfaces; future Lifting Tracker AI feedback | Required | source-doc-cm-design.md §6.6 |
| **`console.log → stderr` guard for MCP stdio** | Three-line redirect of console output to stderr in any MCP stdio server | Every MCP stdio server in the portfolio | Required | source-doc-cm-design.md §6.6 |
| **Per-implement weight + limb-config volume math (D14, D15)** | Internal data convention for weight entry and volume computation | Lifting Tracker analytics | Required | architecture_v0.4.0.md §D14, §D15 |
| **Three-level rest defaulting cascade (D16)** | Session → exercise → set rest_seconds defaulting | Lifting Tracker logging | Required | architecture_v0.4.0.md §D16 |
...
| **CONVENTIONS_v0.2.2.md frontmatter + footer rule** | Every doc carries YAML frontmatter (author / created / updated / tier / content_class / version / status as required by class) and `© YYYY Eric Riutort. All rights reserved.` footer | All in-scope docs | Required | CONVENTIONS_v0.2.2.md §7, §11 |

## 4. Declined standards (presence prevents accidental re-adoption)

| Standard | Scope | Why declined | Source |
|---|---|---|---|
| **Grafana Labs LGTM stack** (Loki + Grafana + Tempo + Mimir) | Observability backend | AGPL 3.0 since 2021; fails composite-principle's permissive-license preference | architecture_v0.4.0.md (Observability); R-TV-03 historical context |
| **WatermelonDB** | Offline-first reactive database for React Native | "Most-commonly-used" argument lost to composite-principle alignment with SQLite + Drizzle (maximum AI-codegen fluency, zero abstraction layer) | architecture_v0.4.0.md §D8 (Sprint 0a revision) |
| **PowerSync Open Edition (for MVP)** | Sync layer for offline-first | Considered as alternate; declined for MVP; revisit when multi-user coach-athlete collaboration becomes real (Sprint 2+) | architecture_v0.4.0.md §D8 (Sprint 0a revision); R-TV-02 |
...
| **Standalone vector DB (Pinecone, Weaviate, etc.)** | Vector storage | pgvector co-located in Postgres is sufficient at this scale; second system not earned | architecture_v0.4.0.md (Three-layer data architecture) |
| **Bespoke skill blueprint framework (replacing Anthropic SKILL.md)** | Skill scaffolding | Replacing an open standard with a bespoke blueprint is a lock-in mistake; Anthropic Agent Skills is the open standard | source-doc-cm-design.md §6.0a (Agent Architect disposition) |
| **Server-side Pydantic enforcement of `ai_interactions.output_episode`** | Typed schema enforcement | Resolved as documentation-only typed-episode contracts (Q15); enforcement at application-code layer | source-doc-cm-design.md §4.9 (v0.3.0 amendments 2) |
| **Folder-per-tier doc layout** | Document filesystem organization | Tier is metadata, not folder; folder moves break links and git blame continuity | source-doc-cm-design.md §3.6 |
