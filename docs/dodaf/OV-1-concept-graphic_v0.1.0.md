...
**MCP-first sub-system strategy made visible.** Each XRSize4 ALL sub-system ships an MCP server before further UI investment. `document-cm` is the first one (governs source documents portfolio-wide). `lifting-tracker-mcp` is planned for the sprint after the athlete MVP ships, exposing tools like `query_sessions`, `log_session`, `get_coach_hierarchy`, `assign_program`. The pattern from `architecture_v0.4.0.md` cross-cutting principles is operationally visible here.

**External services kept separate.** Frontier LLM API, Apple platform services, Stripe (post-alpha), and external video hosts are external — each one a vendor-risk surface tracked in `risks_v0.1.0.md`. The diagram shows them outside the portfolio boundary so the dependency direction is unambiguous.
