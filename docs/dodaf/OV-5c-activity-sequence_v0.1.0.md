...
Per D28 fit-for-purpose: this view shows sequences, not activity decomposition. The activity catalog itself lives in CV-capabilities and `themes-epics-features_v0.2.0.md`. OV-5c picks the activities whose timing and conditional logic actually matter.
...
**Trigger:** A session proposes an edit to `docs/architecture_v0.4.0.md` (or any architecture-class doc).
**Actor (D3 RBAC):** Eric (Super Admin authority over governance) + Claude (under Authority Rule) + the document-cm script suite.
**Stories:** Cross-references source-doc-cm-design.md WF-003. Not in `user-stories_v0.2.0.md` because document-cm is a portfolio governance concern, not a Lifting Tracker user-facing feature.
...
sequenceDiagram
    ...
    Eric->>Claude: "update architecture_v0.4.0.md to add D-NN"
    Claude->>SK: cm.update(doc=architecture_v0.4.0.md, proposal=...)

    SK->>BB: extract baseline (current doc state)
    BB->>FS: read architecture_v0.4.0.md
    FS-->>BB: bytes
    ...
    SK->>FS: write architecture_v0.4.0.md (the actual edit)
    FS-->>SK: written
