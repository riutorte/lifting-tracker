...
| AV-2 Integrated Dictionary | Glossary of terms used across views | table (markdown), no diagram required | `docs/dodaf/lift-track-dodaf-AV-2-dictionary_v0.3.0.md` |
...
| 0.2.5 (planned — Sprint 0d2 CC-4) | TBD | **Drafted, not yet landed.** Sprint 0d2 will land four amendments per `reach4all://docs/research/0d2-prep-amendments-and-sd-entries.md` §4: synthesize-across as work-mode-3 sub-pattern (additive within existing §14.5); three-layer DoDAF view layering; audit-class artifact version-suffix rule; decision-history traceability discipline. **Sprint-open reconciliation required:** the prep doc proposes new §11.7 / §11.8 / §11.9 slots, but those numbers are currently occupied by "What gets which views" / "Tool fallback options" / "Presentation portability" — collision must be resolved at sprint open (renumber existing 11.7-11.9 to 11.10-11.12, or push new amendments to §11.10-§11.12). §14.5 amendment is purely additive (no collision). Per Q9.4 of the prep doc, the binding date for Strategic Implications discipline + decision-history traceability discipline is also a sprint-open call. The audit-class version-suffix rule means CONVENTIONS itself migrates to xrsize4all as `conventions_v0.2.0.md` (version reset on migration per prep §6.2) — coordinate with CC-6 migration sweep. See `lifting-tracker://docs/lift-track-kanban-sprint-0d2.md` CC-4 for full scope. |
