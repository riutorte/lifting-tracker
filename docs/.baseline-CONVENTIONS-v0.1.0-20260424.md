...
- **No version in filename.** Version lives in frontmatter, not the filename. One current exception: `lift-track-source-document-cm_v0.3.0.md` is growing large enough that a split may be warranted; revisit when it crosses that line.
- **Baseline snapshots.** `.baseline-<state>-YYYYMMDD.md` at the same directory as the file they shadow. Examples: `.baseline-lift-track-source-document-cm-v0.2.0-20260423.md`, `.baseline-lift-track-architecture-pre-sprint-0b-20260424.md`.

## 7. Required frontmatter fields per content class

Every in-scope doc carries YAML frontmatter. Fields marked required below must be populated; optional fields are listed in the `lift-track-source-document-cm_v0.3.0.md` manifest schema. Every doc also closes with the copyright footer per §11.
